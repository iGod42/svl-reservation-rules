...
describe("opening hours rule", () => {
	...
	it("rejects times between 7:00 and 20:00 that are not at 00:00", () => {
		expect(typeof srv({...minReservation, Stunde: getDate(10, 20)})).toBe("string")
		
	})
	it("rejects times before 7:00", () => {
		expect(typeof srv({...minReservation, Stunde: getDate(4)})).toBe("string")
	})
	it("rejects times after 20:00", () => {
		expect(typeof srv({...minReservation, Stunde: getDate(22)})).toBe("string")
	})
})