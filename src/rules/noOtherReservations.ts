...
class NoOtherReservations implements Rule {
	...
	private filter = (allReservations: Reservation[]) => (
		hour: Date,
		courtId: number
	) =>
		allReservations.find(
			er => er.hour.getTime() === hour.getTime() && er.courtId === courtId
		)

	evaluate = ({ reservation, allReservations }: RuleEvaluationOptions) =>
		allReservations &&
		this.filter(allReservations)(reservation.hour, reservation.courtId)
			? this.message()
			: undefined
	...
	evaluateBulk = ({
		...
		allReservations?.forEach(res => {
			const theResInfo = relevantResInfo.find(
				ri =>
					ri.hour.getTime() === res.hour.getTime() && ri.courtId === res.courtId
			)
			if (theResInfo) theResInfo.violation = this.message()
		})
	}
}
